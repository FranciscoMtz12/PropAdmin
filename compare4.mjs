...
async function shot(url, path, opts={}) {
  ...
  if (opts.clickTab) {
    const btn = await page.$(`text=${opts.clickTab}`)
    if (btn) { await btn.click(); await page.waitForTimeout(1200) }
  }
  if (opts.expandFirst) {
    const rows = await page.$$('[style*="cursor: pointer"]')
    if (rows[0]) { await rows[0].click(); await page.waitForTimeout(600) }
  }
...
await Promise.all([
  shot(`${SITE}/payments`, '/tmp/pay-svc.png'),
  shot(`${SITE}/payments`, '/tmp/pay-svc-exp.png', { expandFirst: true }),
  shot(`${SITE}/payments`, '/tmp/pay-rpt.png', { clickTab:'Reportes de compras' }),
  shot(`${SITE}/payments`, '/tmp/pay-manual.png', { clickTab:'Manuales' }),
])
console.log('Done.')
await browser.close()
