...
export default function FeedbackPage() {
  ...
  async function loadFeedback() {
    ...
    if (userIds.length > 0) {
      const { data: usersData, error: usersError } = await supabase
        ...
        .in("id", userIds);
      if (usersError) {
        console.error("app_users join failed", usersError);
      } else {
        userMap = new Map((usersData || []).map((u) => [u.id, { full_name: u.full_name, email: u.email }]));
      }
    }
    ...
    const mapped: FeedbackRow[] = raw.map((r) => {
      ...
      return {
        ...r,
        userLabel: u?.full_name || u?.email || "Desconocido",
      };
    });
  ...
  if (loading || loadingData) {
    return (
      <PageContainer>
        <div style={{ padding: "32px 0", color: "var(--text-muted)" }}>Cargando buzón...</div>
      </PageContainer>
    );
  }
          ...
          columns={[
            {
              ...
              header: "Tipo",
              render: (row) => (
                <AppBadge variant={TYPE_VARIANT[row.type]}>
            ...
            },
            {
              key: "user",
              header: "Usuario",
              render: (row) => <span style={{ fontSize: 13 }}>{row.userLabel}</span>,
            },
            {
              key: "page",
              header: "Página",
            ...
            },
            {
              ...
              render: (row) => (
                ...
                >
                  <AppBadge variant={STATUS_VARIANT[row.status]}>
