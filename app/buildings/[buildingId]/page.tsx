  ...
  return (
    <AppCard style={{ padding: 16, borderRadius: 14 }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
        <div>
          <p style={{ fontSize: 13, color: "#667085", marginBottom: 6 }}>{label}</p>
...
export default function BuildingDetailPage() {
  ...
  useEffect(() => {
    if (user?.company_id && buildingId) loadBuilding();
  }, [user, buildingId]);
  ...
  async function loadBuilding() {
    ...
    const [{ data: filesData }, { data: unitsData }, { data: unitTypesData }] = await Promise.all([
      supabase
        .from("building_files")
  ...
  const documentFiles = useMemo(() => files.filter((file) => file.file_type === "document"), [files]);
  const imageFiles = useMemo(() => files.filter((file) => file.file_type === "image"), [files]);

  const occupiedUnits = unitStatuses.filter((unit) => unit.status === "RENTED").length;
  const vacantUnits = unitStatuses.filter((unit) => unit.status === "VACANT").length;
  const maintenanceUnits = unitStatuses.filter((unit) => unit.status === "MAINTENANCE").length;
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        actions={
          <>
            ...
            <UiButton href={`/buildings/${building.id}/unit-types`}>Tipologías</UiButton>
      ...
      <AppGrid minWidth={220} gap={16} style={{ marginBottom: 24 }}>
        <MetricCard label="Categoría" value={categoryDefinition.label} icon={<Tags size={18} />} helper="Clasificación" />
        <MetricCard label="Tipologías" value={unitTypeCount} icon={<Layers3 size={18} />} helper="Modelos de unidad" />
        <MetricCard label="Departamentos" value={totalUnits} icon={<Home size={18} />} helper="Unidades registradas" />
        <MetricCard label="Documentos" value={documentFiles.length} icon={<FileText size={18} />} helper="Archivos técnicos" />
      </AppGrid>
      ...
      {activeTab === "overview" ? (
        <div style={{ display: "grid", gap: 24 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1.1fr 0.9fr", gap: 24 }}>
            <div style={{ display: "grid", gap: 24 }}>
              <SectionCard title="Información general" subtitle="Datos base del edificio y su clasificación." icon={<Building2 size={18} />}>
            ...
            <SectionCard title="Accesos rápidos" subtitle="Navega a los módulos más usados de este edificio." icon={<Layers3 size={18} />}>
              <div style={{ display: "grid", gap: 12 }}>
                <AppCard style={{ padding: 16, borderRadius: 14 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                    <div>
                      <strong style={{ display: "block", marginBottom: 4 }}>Tipologías</strong>
                      <p style={{ margin: 0, color: "#667085", fontSize: 14 }}>Administra los tipos de unidad y sus assets base.</p>
                    </div>
                    <UiButton href={`/buildings/${building.id}/unit-types`}>Abrir</UiButton>
                  </div>
                </AppCard>
                ...
                <AppCard style={{ padding: 16, borderRadius: 14 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                    <div>
                      <strong style={{ display: "block", marginBottom: 4 }}>Departamentos</strong>
                      <p style={{ margin: 0, color: "#667085", fontSize: 14 }}>Consulta ocupación, unidades y assets asignados.</p>
                    </div>
                    <UiButton href={`/buildings/${building.id}/units`} variant="primary">Abrir</UiButton>
                  </div>
                </AppCard>
              </div>
            </SectionCard>
      ...
      {activeTab === "documents" ? (
        <SectionCard title="Documentos" subtitle="Planos, PDFs y otros archivos técnicos del edificio." icon={<FolderOpen size={18} />}>
          {documentFiles.length === 0 ? (
            <p style={{ margin: 0, color: "#667085" }}>Todavía no hay documentos registrados para este edificio.</p>
          ) : (
            <div style={{ display: "grid", gap: 12 }}>
              {documentFiles.map((file) => (
                <AppCard key={file.id} style={{ padding: 16, borderRadius: 14 }}>
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                    <AppIconBox size={40} radius={12} background="#EFF6FF" color="#2563EB">
                      <FileText size={18} />
                    </AppIconBox>
      ...
      {activeTab === "gallery" ? (
        <SectionCard title="Galería" subtitle="Renders y fotografías del edificio en una misma vista visual." icon={<FileImage size={18} />}>
          {imageFiles.length === 0 ? (
            <p style={{ margin: 0, color: "#667085" }}>Todavía no hay imágenes registradas para este edificio.</p>
          ) : (
            <AppGrid minWidth={260} gap={12}>
              {imageFiles.map((file) => (
                <div key={file.id} style={{ border: "1px solid #E5E7EB", borderRadius: 14, overflow: "hidden", background: "white" }}>
                  {file.public_url ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={file.public_url} alt={file.file_name} style={{ width: "100%", height: 180, objectFit: "cover", display: "block" }} />
                  ) : (
                    <div style={{ height: 160, display: "flex", alignItems: "center", justifyContent: "center", background: "#F8FAFC", color: "#94A3B8" }}>
                      <ImageIcon size={22} />
                    </div>
                  )}
                  <div style={{ padding: 14 }}>
                    <strong style={{ display: "block", marginBottom: 4 }}>{file.file_name}</strong>
                    <p style={{ margin: 0, color: "#667085", fontSize: 14 }}>{getFileCategoryLabel(file.file_category)}</p>
                  </div>
                </div>
              ))}
            </AppGrid>
