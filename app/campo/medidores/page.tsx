...
export default function CampoMedidoresPage() {
  ...
  async function loadData() {
    ...
    if (unitIds.length > 0) {
      const { data: uData } = await supabase.from('units').select('id, unit_number').in('id', unitIds);
    ...
    imList.forEach(im => {
      ...
      im.building_id = cfe?.building_id;
    ...
    const grps: Group[] = [];
    cfeMList.forEach(cfe => {
      const meters = imList.filter(im => im.cfe_meter_id === cfe.id);
                      ...
                      >
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, flexWrap: "wrap" }}>
                          <div>
                            ...
                            <p style={{ margin: "2px 0 0", fontSize: 13, color: "var(--text-muted)" }}>{im.tenant_name || "Vacante"}</p>
                          </div>
                          <span style={{
