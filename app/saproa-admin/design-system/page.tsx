...
type ViewportSize = 375 | 768 | 1280 | null; // null = desktop completo
...
// Orden: Super Soft → Clásico → Rígido (de más a menos redondeado)
...
export default function DesignSystemPage() {
  ...
  // ── Cambio de tema sincroniza slider ─────────────────────────────────
  ...
  // ── Slider manual → modo clásico custom ──────────────────────────────
  ...
  const catalogMaxWidth  = viewport !== null ? `${viewport}px` : "none";
  ...
  const handleGenerateReport = useCallback(() => {
    ...
    const radiusDisplay =
      theme === "rigido"
        ...
        : `${radius}px (Clásico custom)`;
    const viewportDisplay =
      viewport === null
        ? "Desktop (ancho completo)"
        : `${viewportPreset.label} — ${viewport}px`;
    ...
    <PageContainer>
      ...
      <style>{`
        ...
        @media (max-width: 900px) {
          .ds-outer-layout { flex-direction: column-reverse !important; }
          .ds-controls-sidebar { width: 100% !important; min-width: 0 !important; position: static !important; max-height: none !important; }
        }
      `}</style>
      ...
      {/* ── CSS vars scopeadas: cubre catálogo + panel de controles ── */}
      <div style={{ ...(catalogVars as unknown as CSSProperties), transition: "all 0.3s ease" }}>

        {/* ════════════════════════════════════════════════════════════
            LAYOUT PRINCIPAL: [catálogo] + [sidebar controles derecha]
        ════════════════════════════════════════════════════════════ */}
        <div
          className="ds-outer-layout"
          style={{ display: "flex", gap: "20px", alignItems: "flex-start" }}
        >

          {/* ═══════════════════════════════════════════════════════
              ÁREA CENTRAL — viewport selector + catálogo
          ═══════════════════════════════════════════════════════ */}
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            >
              ...
              >
                ...
                >
                  <p style={{ fontSize: "0.6875rem", fontWeight: 600, color: "var(--text-muted)", marginBottom: "8px" }}>
                  ...
                  >
                    <EntityCard
                      ...
                      actions={
                        <UiButton variant="secondary" style={{ fontSize: "0.75rem", padding: "6px 10px" }}>
                          Ver
                        </UiButton>
                      }
                    />
                    ...
                    {["Todos", "Activos", "Pendientes", "Archivados", "Con deuda", "Sin inquilino"].map((f, i) => (
                      <button
                        ...
                        style={{
                          ...
                          border: i === 0 ? `1px solid ${accent}` : "1px solid var(--border-default)",
                ...
                <CatalogSection title="Modales" componentNames={["Modal"]}>
                  ...
                  <Modal
                    ...
                    subtitle="Responde a radius, acento y modo del sandbox"
                    onClose={() => setShowModal(false)}
                    maxWidth="480px"
                  >
                    <p style={{ color: "var(--text-muted)", fontSize: "0.875rem", marginBottom: "16px", lineHeight: 1.6 }}>
                      Los componentes dentro del modal heredan las CSS vars del sandbox.
                    </p>
                ...
                >
                  ...
                  >
                    ...
                    />
                    <AppCard>
                      <p style={{ fontSize: "0.8125rem", fontWeight: 700, color: "var(--text-secondary)", marginBottom: "14px" }}>
          ...
          {/* ═══════════════════════════════════════════════════════
              SIDEBAR DERECHA — panel de controles (sticky)
          ═══════════════════════════════════════════════════════ */}
          <div
            className="ds-controls-sidebar"
            style={{
              width: "264px",
              minWidth: "264px",
              position: "sticky",
          ...
          >
            <AppCard style={{ padding: "18px" }}>
              ...
              {/* ── Border-radius (sincronizado con temas) ─── */}
              ...
              <div style={{ marginBottom: "18px" }}>
                <CtrlLabel
                  ...
                  value={`${spacing}px`}
                  hint="Gap entre ítems del catálogo"
                />
                <input
                  type="range"
                  className="ds-range"
              ...
              {/* ── Color de acento — rediseñado ─── */}
              <div style={{ marginBottom: "18px" }}>
                <CtrlLabel
                  label="Color de acento"
                  value={
                    accent === ACCENT_BASE.value
                      ? "SAPROA"
                      : ACCENT_COMPANIES.find((o) => o.value === accent)?.company
                        ?? "Custom"
                  }
                />

                {/* Base SAPROA */}
                <p style={{ fontSize: "0.6875rem", fontWeight: 600, color: "var(--text-muted)", marginBottom: "6px" }}>
                  Base plataforma
                </p>
                <div style={{ marginBottom: "10px" }}>
                  <button
                    type="button"
                    title={`SAPROA — ${ACCENT_BASE.value}`}
                    onClick={() => setAccent(ACCENT_BASE.value)}
                    style={{
                      width: "32px", height: "32px",
                      borderRadius: "50%",
                      background: ACCENT_BASE.value,
                      border: accent === ACCENT_BASE.value
                        ? "3px solid var(--text-primary)"
                        : "2px solid var(--border-default)",
                      cursor: "pointer",
                      transform: accent === ACCENT_BASE.value ? "scale(1.18)" : "scale(1)",
                      boxShadow: accent === ACCENT_BASE.value ? `0 0 0 3px ${ACCENT_BASE.value}40` : "none",
                  ...
                  {ACCENT_COMPANIES.map((opt) => (
                    <button
                      ...
                      style={{
                        ...
                        boxShadow: accent === opt.value ? `0 0 0 3px ${opt.value}40` : "none",
                        transition: "transform 0.15s, box-shadow 0.15s",
                      }}
                    />
                  ))}
                  ...
                  {customColor && (
                    <button
                      ...
                      style={{
                        ...
                        border: accent === customColor
                          ? "3px solid var(--text-primary)"
                          : "2px solid var(--border-default)",
                        cursor: "pointer",
                        transform: accent === customColor ? "scale(1.18)" : "scale(1)",
                        boxShadow: accent === customColor ? `0 0 0 3px ${customColor}40` : "none",
                        transition: "transform 0.15s, box-shadow 0.15s",
                        flexShrink: 0,
                      }}
                    />
                ...
                <p style={{ fontSize: "0.6875rem", color: "var(--text-muted)", marginTop: "6px" }}>
                  {accentLabel(accent, customColor)}
                </p>
              </div>
              ...
              <div style={{ marginBottom: "18px" }}>
                ...
                <p style={{ fontSize: "0.6875rem", color: "var(--text-muted)", marginTop: "4px" }}>
                  Seleccionar tema mueve el slider al radius correspondiente.
                </p>
              </div>
              ...
              <div style={{ marginBottom: "18px" }}>
                ...
                <div
                  style={{
                    ...
                    padding: "5px 8px",
                    borderRadius: "var(--border-radius-sm)",
                    background: isDark ? "rgba(255,193,7,0.1)" : "rgba(99,102,241,0.08)",
                    border: `1px solid ${isDark ? "rgba(255,193,7,0.25)" : "rgba(99,102,241,0.2)"}`,
                    display: "flex",
                    alignItems: "center",
                    gap: "4px",
                  }}
                >
                  ...
                  <span style={{ fontSize: "0.6875rem", fontWeight: 600, color: "var(--text-secondary)" }}>
                    Solo previsualización — no afecta la app
                  </span>
                </div>
              </div>
