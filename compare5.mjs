...
async function shotWithExpand(tab, outPath) {
  ...
  // Click first item card (the first ITEM_CARD div) — use the concept icon span as anchor
  const firstCard = page.locator('[style*="border-radius: 12px"]').first()
  if (await firstCard.count()) {
    await firstCard.click({ position:{ x:200, y:20 } })
...
async function shot(tab, outPath) {
  const ctx = await browser.newContext({ viewport:{ width:1280, height:900 } })
  await ctx.addInitScript(({ key, value }) => { localStorage.setItem(key, JSON.stringify(value)) }, session)
  const page = await ctx.newPage()
  await page.goto(`${SITE}/payments`, { waitUntil:'networkidle', timeout:30000 })
...
await Promise.all([
  shot(null, '/tmp/svc-collapsed.png'),
  shotWithExpand(null, '/tmp/svc-expanded.png'),
  shot('Reportes de compras', '/tmp/rpt.png'),
  shotWithExpand('Manuales', '/tmp/manual-expanded.png'),
])
console.log('Done.')
await browser.close()
