...
export default function BuildingUtilitySubMetersModal({
  meter,
  units,
  existingSubMeters,
  onClose,
  ...
  async function handleSave() {
    ...
    for (const unit of sortedUnits) {
      ...
      if (!state.checked && state.existingId !== null) {
        ...
        if (error) {
          setErrorMsg(`Error al eliminar submedidor: ${error.message}`)
          setSaving(false)
          return
        }
        ...
        if (error) {
          setErrorMsg(`Error al crear submedidor: ${error.message}`)
          setSaving(false)
          return
        }
      } else if (state.checked && state.existingId !== null) {
        ...
        const { error } = await supabase
          ...
          .eq("id", state.existingId)
        if (error) {
          setErrorMsg(`Error al actualizar submedidor: ${error.message}`)
          setSaving(false)
          return
        }
      }
  ...
  return (
    <Modal open title={modalTitle} onClose={onClose} maxWidth="640px">
      ...
      >
        ...
        <span>
          Marca las unidades conectadas a este medidor. La lectura base puede ser 0 si no la conoces.
        </span>
      </div>

      {/* Error banner */}
      {errorMsg && (
        <div style={{ ...errorBannerStyle, marginBottom: 16 }}>{errorMsg}</div>
      )}
      ...
      >
        <UiButton variant="secondary" onClick={onClose} disabled={saving}>
          Cancelar
        </UiButton>
